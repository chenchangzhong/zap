...
实测 `late` 全程 1~15ms(心跳准时),说明主线程**没有**被"占住不让事件循环跑"。

### 根因

动画期间**每帧都要重建整棵元素树** —— `warp_update_layer` 里的 `build_scene`(元素树
layout + paint)。浮层里挂着完整编辑器,这一帧实测 **26.1ms**,远超 16.7ms 的帧预算,帧率掉到
约 38fps,观感就是"卡"。

同一次测量里,下游的 `Renderer::render`(取 drawable + 编码提交)只有 **0.5ms**;
`nextDrawable` 的 10~22ms 也只是**正常的 vsync 等待**。所以瓶颈既不在 GPU,也不在"创建编辑器"。

定位过程值得记下来,因为我先后猜错了三次:

| 猜测 | 怎么被证伪 |
|---|---|
| `CodePane::new` 同步加载文件 | 探针实测 **0.8ms** —— 它只注册一个惰性构造闭包,真正读文件发生在 `CodeView` 首次被构建时 |
| 高亮解析要过一遍整个文件 | 高亮是**按可见范围**查询 + 缓存的(`syntax_tree::highlights_in_ranges`),纯文本更是直接跳过 |
| GPU 侧吞吐不足 | 采样确实显示主线程在 `nextDrawable` 上等待,但那是 vsync;`encode+submit` 只有 0.5ms |

真正把范围收窄的是两个**对照实验**(成本极低,却砍掉一半假设):

- **没动画时不卡** —— 若瓶颈是"首次渲染",没动画时同样要付,应该也卡;
- **每次都卡**(而不是第一次冷缓存)。

最后把探针放到 `build_scene` 上,一次就测到了 26ms。

### 解法:让动画期间的元素树保持小

关键认识:**动画期间的成本与"元素树里有什么"成正比**,因为每帧都要重新 layout + paint 整棵树。
所以最有效的做法是让动画期间**根本不放编辑器**:

- 过渡期间只渲染卡片背景 + 遮罩(+ 一个加载占位);
- **编辑器推迟到动画结束之后才创建**(懒创建)—— 这样"读文件 + 建 buffer + 首次布局"整套都落在
  动画之后;
- **"浮层可见"的判据要同时覆盖两种状态**(编辑器就绪 / 等待创建),否则等待期浮层不渲染;
- 等待期渲染加载占位,别让用户对着空白卡片干等(这里直接复用了 code review 面板的
  `render_loading_state`,视觉天然一致)。

注意:**"动画期间不渲染内容"和"动画期间不创建内容"是两件事**。只做前者(把 `ChildView` 换成
空壳)也能让 `build_scene` 变快 —— 但如果创建仍发生在动画期间,创建时的那一帧照样会拖慢动画。
...
3. **`set_zoom_factor` 不能拿来做单元素缩放。** 它是应用级的:改的是 presenter 的
   ...
   布局、重新光栅化字体,还要换算事件坐标。
4. **探针本身会掩盖要测的问题。** 每帧 `eprintln!` 会改变帧调度:加上每帧打印后,
   `nextDrawable` 只等 10~22ms、一条慢帧都没有;去掉打印改用 `sample` 采样,才抓到 ~190ms
   的等待。**探针必须设阈值、平时零 I/O**,否则你会得到"一切正常"的假象。
5. **改一个判据时,把所有用它的地方一起过一遍。** 修"收起期间浮层仍要渲染"时,只在
   `dismiss` 里动了手,却忘了渲染判据用的是同一个 `floating_editor_visible()` —— 结果收起
   动画变成"啪地消失"。
