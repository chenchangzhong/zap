...
use crate::util::git::{
    ...
    run_git_command_bytes_with_stdin, run_git_network_command, Commit, PrInfo,
};
...
pub struct FileDiffAndContent {
    ...
    /// diff base(HEAD 或 merge-base)处的完整文件内容,用于 code review editor
    /// 渲染 inline diff(`set_base`)。
    ///
    /// `None` 表示没有可用 baseline,不为该文件构造 editor:二进制文件、
    /// 非文件条目(如嵌套 repo/worktree 目录)、`git show` 失败,或内容从未加载
    /// /在传输中被截留(从缓存的 `GitDiffData` 重建、超预算文件)。
    ///
    /// `Some("")` 表示 baseline 存在但为空:base 中不存在的新/untracked 文件
    /// (diff 正确渲染为全部新增)或 base 提交中确实为空的文件。
...
impl DiffStateModel {
    ...
    pub async fn diff_metadata_against_head(repo_path: &Path) -> Result<DiffMetadataAgainstBase> {
        ...
        for (file_path, status) in &changed_files {
            if let Some(metadata) = num_stat_metadata.get(file_path) {
                ...
                // 逐条目降级:不可计数的 untracked 路径(如嵌套 repo/worktree 目录,
                // 或不可读文件)贡献 0 行,而不是让整个 metadata 计算失败。
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        // baseline 一次批量取回,替代逐个 `git show HEAD:<path>`(实测 ≈7ms/文件)。
        let baselines = Self::baselines_for_files(repo_path, "HEAD", &changed_files).await;

        for ((file_path, status), content_at_head) in changed_files.into_iter().zip(baselines) {
            ...
            Some(base) => {
                match status {
                    GitFileStatus::New | GitFileStatus::Untracked => {
                        // 非文件条目(如嵌套 repo/worktree 目录)不构造 baseline,
                        // 因此不会为它们创建 editor。
    ...
    /// 解析 `git cat-file --batch` 的输出:`<oid> <type> <size>\n<内容>\n`,
    /// 或 `<object> missing\n`(对象不存在)。返回与请求同序的结果。
    ///
    /// 只接受 `blob`(树不在 baseline 的取值范围内,返回 `None` 更安全);缺失、截断、
    /// 非 blob 等任何异常都退化为 `None` —— 与逐个 `git show` 失败时的语义一致。
    pub(crate) fn parse_cat_file_batch_output(output: &[u8], count: usize) -> Vec<Option<String>> {
        let mut results = Vec::with_capacity(count);
        let mut cursor = 0usize;

        while results.len() < count {
            let Some(line_len) = output[cursor..].iter().position(|byte| *byte == b'\n') else {
                break;
            };
            let header = String::from_utf8_lossy(&output[cursor..cursor + line_len]).to_string();
            cursor += line_len + 1;

            // 头部形如 "<oid> blob <size>";末尾字段解析不出数字(如 "missing"/"ambiguous")
            // 的条目没有内容段。注意:非 blob(树等)同样**带**内容段,必须一并跳过,
            // 否则后续分帧会错位。
            let mut fields = header.rsplitn(3, ' ');
            let size = fields.next().and_then(|field| field.parse::<usize>().ok());
            let is_blob = fields.next() == Some("blob");
            ...
            if cursor + size > output.len() {
                break; // 输出被截断,后续无法可靠分帧。
            }
            let content = &output[cursor..cursor + size];
            cursor += size + 1; // 跳过内容末尾的 '\n'

            // 树不在 baseline 的取值范围内,内容跳过但结果记为 None。
            results.push(is_blob.then(|| String::from_utf8_lossy(content).to_string()));
        }
    ...
    /// 需要 baseline 的 revspec 路径:重命名的内容在旧路径上,其余在新路径上。
    fn baseline_revspec_path(file_path: &PathBuf, status: &GitFileStatus) -> String {
        match status {
            GitFileStatus::Renamed { old_path } => old_path.clone(),
            _ => file_path.to_string_lossy().to_string(),
        }
    }

    /// 一次 `git cat-file --batch` 取回多个文件的 baseline 内容,替代逐个
    /// `git show <commit>:<path>`(实测 ≈7ms/文件;已跟踪改动多的仓库里,这批 spawn 是
    /// 打开审核面板关键路径的大头)。`requests` 是 revspec 的路径部分(按序),返回同序结果。
    async fn read_baseline_contents_batch(
        repo_path: &Path,
        commit: &str,
        requests: &[String],
    ) -> Vec<Option<String>> {
        ...
        match run_git_command_bytes_with_stdin(
            repo_path,
            &["cat-file", "--batch"],
            stdin.into_bytes(),
        )
        .await
        {
            Ok(output) => Self::parse_cat_file_batch_output(&output, requests.len()),
            Err(err) => {
                // 取不到内容时退化为「无 baseline」,而不是让整次加载失败 ——
                // 与逐个路径 `git show` 失败的语义一致。
                log::warn!("cat-file --batch failed; baselines will be empty: {err}");
                vec![None; requests.len()]
            }
        }
    }

    /// 为一批改动文件批量取回 baseline(与 `files` 同序)。
    ///
    /// - 未跟踪/新增:不进 git;文件存在时用空串,目录条目(嵌套 repo)仍为 `None` —— 与
    ///   `get_file_content_at_head` / `get_file_content_at_commit` 的同名分支一致;
    /// - 重命名:内容在旧路径上;
    /// - 其余:在新路径上。
    ///
    /// **含换行的路径不能走批量协议**:`cat-file --batch` 的请求按 `\n` 分隔,路径里的 `\n`
    /// 会被拆成两个请求,多出的响应让后续请求整体错位 —— 实测后果是这些文件拿到**别的文件的
    /// 内容**当 baseline(`we\nird.txt` 配到 `we` 的内容),其后的文件还会丢掉 baseline。
    /// 这类路径(Unix 允许,Windows 不允许)退回逐文件 `git show`,即批量前的老路径。
    ///
    /// 已知差异:gitlink(子模块)在 `git show` 下会漂亮打印 commit 信息,而 `cat-file --batch`
    /// 返回 `commit` 类型的原始对象 —— 本函数按「非 blob 即取不到」处理,该文件因此没有
    /// baseline。当前仓库均无 `.gitmodules`;若将来引入子模块,这里需加一条逐文件回退。
    async fn baselines_for_files(
        repo_path: &Path,
        commit: &str,
        files: &[(PathBuf, GitFileStatus)],
    ) -> Vec<Option<String>> {
        let batchable_requests: Vec<String> = files
            .iter()
            .filter(|(_, status)| !status.is_new_file())
            .map(|(path, status)| Self::baseline_revspec_path(path, status))
            .filter(|request| !request.contains('\n'))
            .collect();
        let mut fetched =
            Self::read_baseline_contents_batch(repo_path, commit, &batchable_requests)
                .await
                .into_iter();
        ...
        for (path, status) in files {
            if status.is_new_file() {
                results.push(repo_path.join(path).is_file().then(String::new));
            } else if Self::baseline_revspec_path(path, status).contains('\n') {
                results.push(Self::get_file_content_at_head(repo_path, path, status).await);
            } else {
                results.push(fetched.next().flatten());
            }
        }
        results
    }
    ...
    async fn get_file_diff(
        ...
        // `git status` 可能报告非文件 untracked 条目(如本仓库内检出的嵌套
        // repo/worktree 目录)。目录没有可 diff 的内容 —— `git diff --no-index`
        // 对目录会失败 —— 所以直接返回空非二进制 diff。
