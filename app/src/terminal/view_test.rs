...
use crate::ai::blocklist::{ResponseStream, ResponseStreamId};
        ...
        terminal.update(&mut app, |view, ctx| {
            {
                ...
                model.simulate_block("ls", "foo");
                model.simulate_block("pwd", "bar");
                // 本地 add_window_with_terminal 后首个 block 仍是 bootstrap stage（隐藏）。
                // 显示它使 ls 成为可见的块选择目标，与上游测试环境一致。
                model.block_list_mut().blocks_mut()[0].set_show_bootstrap_block(true);
            }
...
fn simulate_user_shell_block_in_conversation(
    ...
    let block = &mut model.block_list_mut().blocks_mut()[index];
    block.set_conversation_id(conversation_id);
    // 本地 add_window_with_terminal 后首个 block 仍是 bootstrap stage（should_hide_block
    // 会隐藏它，高度 0，无法成为导航点）。显示它使模拟的 shell block 可见，与上游测试
    // 环境（bootstrap 已完成）行为对齐。
    block.set_show_bootstrap_block(true);
    index.into()
}
