//! 冻结 DCS hook 的线上格式：手写反序列化（`DProtoHook` / `BootstrappedValue`）必须与
//! 原先的 serde 派生接受完全相同的输入，这些 payload 会出现在用户 RC 文件片段、
//! 持久化会话与服务端消息里。
...
fn every_hook_tag_dispatches_to_the_matching_variant() {
    let cases = [
        ...
        ("InitShell", serde_json::json!({"session_id": 1, "shell": "zsh"})),
