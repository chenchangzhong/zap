...
impl ActionButtonTheme for OmpButtonTheme {
    ...
    fn text_color(
        &self,
        _: bool,
        _: Option<warp_core::ui::theme::Fill>,
...
impl OmpModelSelector {
    pub fn new(
        ...
        let button = ctx.add_typed_action_view(|ctx| {
            ActionButton::new("omp", OmpButtonTheme)
                .with_icon(Icon::OhMyPiLogo)
                .with_size(ButtonSize::AgentInputButton)
                .with_tooltip("Select omp model")
                .with_tooltip_alignment(TooltipAlignment::Left)
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(OmpModelSelectorAction::ToggleMenu);
                })
        });

        let dropdown = ctx.add_typed_action_view(|_ctx| {
            Menu::<OmpModelSelectorAction>::new()
                .with_drop_shadow()
        ...
        ctx.spawn(
            warpui::r#async::Timer::after(Duration::ZERO),
            |me, _, ctx| {
                if me.selected_model.is_none() {
                    me.trigger_refresh(ctx);
                }
            },
        );
        ...
        Self {
            terminal_view_id,
            registry: OmpModelRegistry::new(&omp_binary),
            omp_binary,
            selected_model: None,
            loading: false,
            menu_is_open: false,
    ...
    fn trigger_refresh(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        ctx.spawn(
            async move {
                ...
                let model = if r.is_ok() {
                    registry.read_current_model().await.ok()
                } else {
                    None
                };
                (r, model, registry)
            ...
            |me, (r, model, registry), ctx| {
                ...
                if let Ok(()) = r {
                    if me.selected_model.is_none() {
                        me.selected_model = model;
                    }
                    let items: Vec<MenuItem<OmpModelSelectorAction>> = me
                        .registry
                        .models()
                        .iter()
                        .map(|m| {
                        ...
                        .collect();
                    let selected = me.selected_model.as_deref().and_then(|s| {
                        me.registry.models().iter().position(|m| m.selector == s)
                    });
    ...
    fn display_label(&self) -> String {
        let Some(ref selector) = self.selected_model else {
            return "Select model".into();
        };
        ...
        self.registry
            ...
            .map(|m| m.name.clone())
            .unwrap_or_else(|| format!("Unknown ({selector})"))
    }
}
...
impl View for OmpModelSelector {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let button_row = Flex::row()
            ...
            .with_child(
                Text::new(label, font_family, LABEL_FONT_SIZE)
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            OmpModelSelectorAction::ToggleMenu => {
                ...
                if self.menu_is_open && self.registry.models().is_empty() {
            ...
            OmpModelSelectorAction::SelectModel { selector } => {
                ...
                ctx.emit(OmpModelSelectorEvent::ModelSelected {
                    selector: sel.clone(),
                });
                // Persist as default model + notify running omp via socket.
                let omp_binary = self.omp_binary.clone();
                ctx.spawn(
                    async move {
                    ...
                    |_me, result, ctx| {
                        if let Err(e) = result {
                            log::warn!("OmpModelSelector: set_model failed: {e}");
                        }
                        ctx.notify();
                    },
                );
