...
pub struct BlockListElement {
    ...
    /// When true, suppresses Zap's cursor overlay and cursor-contrast coloring
...
impl BlockListElement {
    ...
    fn right_mouse_down(
        ...
        // 运行中的全屏程序接管鼠标（开启鼠标上报）时，把原始右键转发给它，
        // 与左键 down/up/drag、滚轮在同一条件下的处理保持一致。
        ...
        // 设置开启时，裸右键直接粘贴；Shift+右键仍打开上下文菜单。
    ...
    fn dispatch_event(
        ...
        handled |= match event_at_z_index {
            ...
            Event::RightMouseDown {
                ...
                self.right_mouse_down(*position, &modifiers, ctx, app)
            }
            Event::LeftMouseUp {
                position,
                modifiers,
