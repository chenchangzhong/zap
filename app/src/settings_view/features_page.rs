...
use crate::settings::{
    ...
    EnableSshAutoDiscovery, EnableSlashCommandsInTerminal,
    ...
    RightClickBehavior, RightClickBehaviorSetting,
    ...
    ShowTerminalInputMessageBar, SshSettings,
...
impl FeaturesPageAction {
    fn telemetry_event(&self, ctx: &AppContext) -> TelemetryEvent {
        ...
        match self {
            ...
            Self::SetRightClickBehavior(right_click_behavior) => TelemetryEvent::FeaturesPageAction {
...
impl SettingsWidget for RightClickBehaviorWidget {
    ...
    fn search_terms(&self) -> &str {
        "right click behavior context menu paste clipboard"
    }
    ...
    fn render(
        ...
        let mut column = Flex::column();
        let selection_settings = SelectionSettings::as_ref(app);
        // 设为粘贴后，裸右键不再弹菜单；此处提示 Shift+右键仍可打开，否则该操作无从发现。
        let paste_hint = crate::t!("settings-features-right-click-behavior-hint");
        add_setting(
            &mut column,
...
impl SettingsWidget for PreserveInputFocusOnBlockSelectionWidget {
    ...
    fn render(
        ...
        let ui_builder = appearance.ui_builder();
        render_body_item::<FeaturesPageAction>(
            crate::t!("settings-features-preserve-input-focus-on-block-selection"),
            None,
            LocalOnlyIconState::for_setting(
