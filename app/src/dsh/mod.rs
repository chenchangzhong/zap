...
//! - `loopback_ipc`:CEF 后端下的回环 HTTP IPC shim(`webkit.messageHandlers.ipc`
//!   的替代通道,协议不变)。
//!
...
// 只随 CEF feature 编译:该模块为 CEF 后端的回环 IPC 而存在,默认构建不应把它
// (及其 dispatch2 依赖)编进来(评审 F15)。
#[cfg(feature = "cef_webview")]
pub(crate) mod loopback_ipc;
pub(crate) mod pane;
pub(crate) mod runtime;

pub use pane::DshPane;
pub use runtime::{
    DshChannelsSnapshot, DshRestartResult, DshRuntime, DshRuntimeStatus, DshStartResult,
    DshUpdateCheck, PollResult,
};
