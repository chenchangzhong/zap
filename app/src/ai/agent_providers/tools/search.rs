...
fn grep_parameters() -> Value {
    json!({
        ...
        "properties": {
            "queries": {
                "type": "array",
                "description": "List of keywords/regex patterns to search for (each element is an independent query; any hit counts as a match).",
                "items": {"type": "string"}
            },
            "path": {
                "type": "string",
...
fn glob_parameters() -> Value {
    json!({
        ...
        "properties": {
            "patterns": {
                "type": "array",
                "description": "List of filename wildcard patterns (supports ?, *, [...]). E.g. [\"**/*.rs\", \"src/**/*.toml\"].",
                "items": {"type": "string"}
            },
            "search_dir": {
                "type": "string",
            ...
            "limit": {
                "type": "integer",
                "description": "Maximum number of results to return; 0 or omitted means no limit.",
                "default": 0
            }
        },
        "required": ["patterns"],
