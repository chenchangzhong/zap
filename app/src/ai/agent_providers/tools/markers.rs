...
pub static OPEN_CODE_REVIEW: OpenAiTool = OpenAiTool {
    ...
    description: "Open the Code Review panel for the current project (client UI signal, no arguments).\
                  Use when the user explicitly asks to open code review, or when the context shows a review phase is starting.",
...
fn transfer_parameters() -> Value {
    json!({
        ...
        "properties": {
            "reason": {
                "type": "string",
                "description": "Explain to the user why control is being handed back (e.g. \"I need you to complete the interactive login manually\")."
            }
        },
        "additionalProperties": false
    })
...
pub static TRANSFER_SHELL_CONTROL: OpenAiTool = OpenAiTool {
    ...
    description: "Hand the PTY control of the current long-running shell command back to the user.\
                  Use when the command needs manual interaction that is not a good fit for write_to_long_running_shell_command\
                  (e.g. interactive login, or steps that require watching the live terminal echo to decide what to do next).\
