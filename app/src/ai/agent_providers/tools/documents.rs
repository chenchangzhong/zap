...
fn read_parameters() -> Value {
    json!({
        ...
        "properties": {
            "documents": {
                "type": "array",
                "description": "List of documents to read (each identified by document_id).",
                "items": {
                    "type": "object",
                    "properties": {
                        ...
                        "line_ranges": {
                            "type": "array",
                            "description": "Optional 1-based inclusive line ranges; empty reads the whole document.",
                            "items": {
                                "type": "object",
                                "properties": {
                                    "start": { "type": "integer" },
...
pub static READ_DOCUMENTS: OpenAiTool = OpenAiTool {
    name: "read_documents",
    description: "Read Zap Drive documents (referenced by document_id, not files on disk).\
...
fn edit_parameters() -> Value {
    json!({
        ...
        "properties": {
            "diffs": {
                "type": "array",
                "description": "One search→replace per document. Each diff describes a single replacement.",
                "items": {
                    "type": "object",
                    "properties": {
                        "document_id": { "type": "string" },
                        "search": {
                            "type": "string",
                            "description": "Exact text to replace (must match the document's current content **exactly**, including whitespace and newlines)."
                        },
                        "replace": {
                            "type": "string",
...
pub static EDIT_DOCUMENTS: OpenAiTool = OpenAiTool {
    ...
    description: "String search-and-replace on existing Zap Drive documents.\
                  ...
                  search must match the current document content **exactly** (including whitespace and newlines), otherwise it fails.",
            ...
            "new_documents": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "content": {
                            "type": "string",
                            "description": "Full document content (markdown / plain text)."
                        }
                    },
...
pub static CREATE_DOCUMENTS: OpenAiTool = OpenAiTool {
    ...
    description: "Create one or more new documents in Zap Drive (each with a title + full content).\
                  Use to persist analysis results, notes, todos, etc. as reusable Drive documents.",
