...
fn parameters() -> Value {
    json!({
        ...
        "properties": {
            ...
            "uses_pager": {
                "type": "boolean",
                "description": "Whether the command may trigger a pager (less/more, etc.). Recommend false; append | cat or similar to avoid blocking.",
                "default": false
            },
            "is_risky": {
                "type": "boolean",
                "description": "Whether the command is dangerous (rm -rf, changing global config, etc.). Set true so the user confirms more visibly.",
                "default": false
            },
            "wait_until_complete": {
                "type": "boolean",
                "description": "Defaults to true (wait for the command to finish before returning; suitable for one-off commands). Must be false for commands that never exit naturally, such as dev servers / background processes / tail -f / interactive REPLs, otherwise the current turn hangs forever waiting for a result. With false, a LongRunningCommandSnapshot is returned immediately and later turns can continue interacting via read/write_to_long_running_shell_command.",
                "default": true
            }
        },
        "required": ["command"],
