...
fn is_blocked_ipv4(v4: Ipv4Addr) -> bool {
    ...
    v4.is_loopback()          // 127.0.0.0/8
        ...
        // 注意：198.18.0.0/15(RFC 2544 性能测试段)故意不在阻止列表内——
        // Clash/Surge 等代理工具的 fake-ip 模式用该段作虚拟地址，DNS 会把
        // 公网域名解析到 198.18.x.x，由 TUN 层 NAT 转发真实流量。阻止它会让
        // webfetch/websearch 在代理环境下全部误杀。该段不是 RFC 1918 私有段，
        // 放行不构成 SSRF 风险(内网地址仍被上面各规则拦截)。
