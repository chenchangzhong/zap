...
pub fn conversation_output_status_from_conversation(
    ...
    match conversation.status() {
        ...
        // `InProgress` is not terminal, but we preserve the existing behavior of
        // reporting a terminal outcome whenever the last exchange has already
        // finished.
        ConversationStatus::InProgress
        | ConversationStatus::Success
        | ConversationStatus::Cancelled => terminal_status_from_last_exchange(conversation),
    }
}
