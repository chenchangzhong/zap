...
impl BlocklistAIController {
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&action_model, move |me, _, event, ctx| {
            ...
            if !should_trigger_follow_up_request {
                ...
                // 解锁 pre-snapshot 窗口里排队的行,免得 action 完成但不触发 follow-up
                // 时行被永久锁住。
            ...
            if me.flush_pending_byop_request_after_finished_action(*conversation_id, ctx) {
                // snapshot 已发出,解锁 pre-snapshot 窗口里排队的行;BYOP flush 分支
                // 提前 return,不能漏掉解锁,否则 PendingLrcAutoQueue 行被永久锁死。
                QueuedQueryModel::handle(ctx).update(ctx, |model, ctx| {
                    model.unlock_pending_lrc_rows(*conversation_id, ctx);
                });
                return;
            ...
            // snapshot 已发出,解锁 pre-snapshot 窗口里排队的行。
            QueuedQueryModel::handle(ctx).update(ctx, |model, ctx| {
                model.unlock_pending_lrc_rows(*conversation_id, ctx);
            });
        });
    ...
    fn send_query(
        ...
        if let Some(slash_command_request) = SlashCommandRequest::from_query(query.as_str()) {
            // 只有 fired queued 行带 `queued_query_id`。对它们,slash command(如排队的
            // `/compact`)保持发到其排队的会话;直接提交仍从当前 UI 选区推导目标。
        ...
        let ai_input = match input_query.input_query {
            InputQueryType::UserSubmittedQueryFromInput {
                ...
                // Resolve the attachment set for this submission. Fired queued rows read from
                // their row-owned attachment set (moved out of the live staging at enqueue
                // time); direct submissions consume the live staging (which `pending_context`
                // no longer attaches implicitly — attachments are resolved here explicitly).
    ...
    pub fn cancel_conversation_progress(
        ...
        // 取消时清掉锁定的 pending-LRC 行,免得取消后残留。follow-up 提交路径也会经过
        // 本函数,但那种场景下的 PendingLrcAutoQueue 行属于仍在该命令上排队的新输入,
        // 不能顺手删掉(否则用户手打的 prompt 无声消失),只对真正的取消 reason 清理。
        if !matches!(reason, CancellationReason::FollowUpSubmitted { .. }) {
            QueuedQueryModel::handle(ctx).update(ctx, |model, ctx| {
