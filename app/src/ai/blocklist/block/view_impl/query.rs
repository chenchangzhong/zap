...
use warpui::{
    elements::{
        Border, ChildAnchor, Container, CornerRadius, Flex, MainAxisAlignment, MainAxisSize,
        ParentAnchor, ParentElement, Radius, Shrinkable, Wrap,
    },
...
use crate::ai::blocklist::block::MouseStateHandle;
...
pub(crate) fn render_query(
    ...
    let avatar = if let Some(timestamp) = query_sent_at {
        appearance.ui_builder().overlay_tool_tip_on_element(
            crate::t!(
                "ai-block-query-timestamp-tooltip",
