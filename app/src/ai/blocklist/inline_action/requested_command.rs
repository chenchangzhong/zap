...
pub struct RequestedCommandView {
    ...
    // the action leaves the pending queue and is no longer retrievable.
...
impl RequestedCommandView {
    ...
    /// the server can't be named (e.g. not installed).
...
/// waiting message when the tool name is also unavailable.
pub(crate) fn mcp_blocked_title_text(tool_name: &str, server_name: Option<&str>) -> String {
...
pub(crate) fn mcp_viewing_detail_title_text(tool_name: &str, server_name: Option<&str>) -> String {
    if tool_name.is_empty() {
