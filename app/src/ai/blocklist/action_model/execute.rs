...
use crate::{
    ...
    terminal::{
        model::session::{
            active_session::ActiveSession, command_executor::shell_quote_arg, ExecuteCommandOptions,
            Session,
        },
...
/// A single file that failed to be read, with a reason-specific message.
#[cfg(feature = "local_fs")]
#[derive(Debug)]
...
pub struct ReadFileContextResult {
    ...
    /// Requested files that could not be read, each paired with a
    /// reason-specific failure message (missing, too large, or unprocessable).
...
///
/// If any files cannot be read, they are included in the `failed_files` field
/// of the result with a reason-specific message.
///
