...
/// Advances the coordinator from the pre-bootstrap `WarpInput` stage to
/// `ScriptExecution` without finishing bootstrap, so the terminal stays
/// pre-bootstrap (the startup-path fallback remains in effect) while a following
/// prompt-only precmd is accepted and applied to the fresh active block.
