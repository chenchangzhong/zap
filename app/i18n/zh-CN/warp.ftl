...
workspace-toast-conversations-deleted = 已删除 { $count } 个对话
workspace-toast-all-conversations-deleted = 已删除所有对话
...
ai-block-query-timestamp-tooltip = 提问时间：{ $timestamp }
...
terminal-open-browser-preview = 打开预览
browser-back-tooltip = 后退
browser-forward-tooltip = 前进
browser-reload-tooltip = 刷新
...
settings-features-preserve-input-focus-on-block-selection = 选择块时保持输入焦点
...
settings-features-global-hotkey-activation-hotkey = 显示/隐藏所有窗口
settings-features-global-hotkey-quake-mode = 专用热键窗口
settings-features-global-hotkey-disabled = 禁用
...
settings-features-ctrl-tab-behavior-cycle-most-recent-session = 在最近使用的会话间循环
settings-features-ctrl-tab-behavior-activate-prev-next-tab = 切换到上一个/下一个标签页
settings-features-right-click-behavior-label = 右键行为：
settings-features-right-click-behavior-hint = 按住 Shift 右键可打开上下文菜单。
settings-features-right-click-behavior-context-menu = 打开上下文菜单
settings-features-right-click-behavior-paste = 从剪贴板粘贴
...
settings-agent-providers-headers-hint = 值支持 { $placeholder } 占位符，发送请求时替换为当前会话 id（例如 OpenCode Go 要求的 x-opencode-session）
settings-agent-providers-header-value-placeholder = 值（可含 { $placeholder }）
...
keybinding-desc-workspace-open-blank-web-preview = 打开空白 Web 预览
dsh-first-use-configure-model = DeepSeek Harness 已就绪。请在设置中配置模型 API Key 后开始使用。
dsh-update-available-toast = DeepSeek Harness 有新版本 { $latest }(当前 { $installed })。点击打开终端并升级。
dsh-update-prerelease-toast = DeepSeek Harness { $channel } 渠道有新版本 { $latest }(当前 { $installed })。点击打开终端并升级。
settings-about-dsh-title = DeepSeek Harness
settings-about-dsh-description = 检查 DeepSeek Harness 的更新。关闭某个渠道后,不再检查或提示该渠道的更新。
settings-about-dsh-installed = 已装版本 { $version }
settings-about-dsh-not-installed = 未在 PATH 中找到 DeepSeek Harness
settings-about-dsh-check-hint = 点击"检查更新"查看各渠道版本
settings-about-dsh-channel-update = { $channel } 渠道最新 { $latest }(有新版本)
settings-about-dsh-channel-current = { $channel } 渠道 { $latest }(已最新)
settings-about-dsh-channel-latest-label = latest 渠道(正式版)
settings-about-dsh-channel-next-label = next 渠道(rc 候选预览)
settings-about-dsh-channel-alpha-label = alpha 渠道(内测)
settings-about-dsh-upgrade-now = 立即升级
dsh-runtime-failed-fix-error = 修复错误
dsh-runtime-failed-fix-error-tooltip = 复制错误信息,并附加到终端 Agent 输入框
...
drive-object-type-agent-workflows = Agent 工作流
...
workspace-conversation-delete-confirm-title = 删除「{ $title }」?
workspace-conversation-delete-confirm-title-multiple = 删除 { $count } 个对话?
workspace-conversation-delete-confirm-title-all = 删除所有对话?
workspace-conversation-delete-confirm-title-default = 删除对话?
workspace-conversation-delete-confirm-description = 此对话将被永久删除,且无法撤销。
workspace-conversation-delete-confirm-description-multiple = 这些对话将被永久删除,且无法撤销。
workspace-conversation-delete-confirm-description-all = 所有对话都将被永久删除,且无法撤销。
workspace-conversation-list-select = 选择
workspace-conversation-list-delete-all = 全部删除
workspace-conversation-list-cancel = 取消
workspace-conversation-list-select-all = 全选
workspace-conversation-list-delete-selected = 删除 ({ $count })
workspace-conversation-list-delete-in-progress-error = 对话进行中时无法删除。
workspace-conversation-list-delete-all-partial = 部分对话未删除(进行中或环境智能体对话)。
...
sftp-error-upload-not-connected = 上传失败: 未连接到服务器
sftp-error-download = 下载失败: { $error }
sftp-error-download-not-connected = 下载失败: 未连接到服务器
sftp-ops-error-connection = 连接错误: { $detail }
sftp-ops-error-operation = 操作错误: { $detail }
sftp-ops-error-local-io = 本地 IO 错误: { $detail }
sftp-ops-error-no-credentials = 未找到凭据: { $detail }
sftp-ops-error-cancelled = 传输已取消
cloud-sync-error-crypto = 加密错误: { $detail }
cloud-sync-error-gist = Gist 错误: { $detail }
cloud-sync-error-provider = 数据提供者错误: { $detail }
cloud-sync-error-serialization = 序列化错误: { $detail }
cloud-sync-error-version-store = 版本存储错误: { $detail }
cloud-sync-gist-error-request = 网络请求失败: { $detail }
...
settings-about-cef-title = CEF 内核
settings-about-cef-description = 从 CEF 上游构建索引检查新版本;升级是手动重建(改 cef crate 版本 → 重嵌 framework → 重签名),见 CEF-UPGRADE.md。
settings-about-cef-installed = 内核 { $version }
settings-about-cef-up-to-date = 已是最新({ $version })
settings-about-cef-available = 有新版本 { $latest }(当前 { $installed })
settings-about-cef-failed = 检查失败:{ $error }
settings-about-cef-unknown = 版本号无法比较
