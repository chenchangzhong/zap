...
ai-block-query-timestamp-tooltip = Message sent { $timestamp }
...
terminal-open-browser-preview = Open preview
browser-back-tooltip = Back
browser-forward-tooltip = Forward
browser-reload-tooltip = Reload
...
settings-features-global-hotkey-activation-hotkey = Show/hide all windows
settings-features-global-hotkey-quake-mode = Dedicated hotkey window
settings-features-global-hotkey-disabled = Disabled
...
settings-features-ctrl-tab-behavior-cycle-most-recent-session = Cycle most recent session
settings-features-ctrl-tab-behavior-activate-prev-next-tab = Activate previous/next tab
settings-features-right-click-behavior-label = Right-click behavior:
settings-features-right-click-behavior-hint = Shift+right-click to open the context menu.
settings-features-right-click-behavior-context-menu = Open the context menu
settings-features-right-click-behavior-paste = Paste from the clipboard
...
settings-agent-providers-headers-hint = Values support the { $placeholder } placeholder — replaced with the current conversation id at request time (e.g. the x-opencode-session header required by OpenCode Go)
settings-agent-providers-header-value-placeholder = Value (may contain { $placeholder })
...
keybinding-desc-workspace-open-blank-web-preview = Open Blank Web Preview
dsh-first-use-configure-model = DeepSeek Harness is ready. Configure your model API key in its settings to start.
dsh-update-available-toast = DeepSeek Harness { $latest } is available (current: { $installed }). Click to open a terminal and upgrade.
dsh-update-prerelease-toast = DeepSeek Harness { $channel } channel has { $latest } available (current: { $installed }). Click to open a terminal and upgrade.
settings-about-dsh-title = DeepSeek Harness
settings-about-dsh-description = Check DeepSeek Harness updates. Turning off a channel skips its check and update toast.
settings-about-dsh-installed = Installed: { $version }
settings-about-dsh-not-installed = DeepSeek Harness not found in PATH
settings-about-dsh-check-hint = Select "Check for update" to see per-channel versions
settings-about-dsh-channel-update = { $channel } channel: { $latest } (update available)
settings-about-dsh-channel-current = { $channel } channel: { $latest } (up to date)
settings-about-dsh-channel-latest-label = "latest" channel (stable release)
settings-about-dsh-channel-next-label = "next" channel (release candidate preview)
settings-about-dsh-channel-alpha-label = "alpha" channel (experimental)
settings-about-dsh-upgrade-now = Upgrade now
dsh-runtime-failed-fix-error = Fix error
dsh-runtime-failed-fix-error-tooltip = Copy the error and attach it to the terminal Agent input
...
# 命名前缀:sftp-*
sftp-pane-title = File Manager
sftp-dialog-current-name = Current name: { $name }
sftp-dialog-delete-title = Confirm delete
sftp-dialog-delete-one = Are you sure you want to delete "{ $name }"? This cannot be undone.
sftp-dialog-delete-many = Are you sure you want to delete { $count } items? This cannot be undone.
sftp-dialog-create-folder-title = New folder
sftp-dialog-create = Create
sftp-dialog-details-title = File details
sftp-dialog-details-type = Type
sftp-dialog-details-size = Size
sftp-dialog-details-modified = Modified
sftp-dialog-details-permissions = Permissions
sftp-dialog-details-path = Path
sftp-dialog-move-title = Move file
sftp-dialog-move-confirm = Move
sftp-dialog-move-description = Move "{ $source }" to { $target }
sftp-dialog-overwrite-title = Confirm overwrite
sftp-dialog-overwrite-confirm = Overwrite
sftp-dialog-overwrite-upload = The remote file { $name } already exists. Overwrite it?
sftp-dialog-overwrite-download = The target file { $name } already exists. Overwrite it?
sftp-dialog-close-transfer-title = Close transfer panel
sftp-dialog-close-transfer-body = Transfers are still in progress. Closing will interrupt them all and clear the record. Close anyway?
sftp-file-type-file = File
sftp-file-type-directory = Directory
sftp-file-type-symlink = Symlink
...
sftp-transfer-status-waiting = Waiting
sftp-transfer-status-active = Transferring
sftp-transfer-status-done = Completed
sftp-transfer-status-failed = Failed
sftp-transfer-status-cancelled = Cancelled
sftp-transfer-panel-title = Transfers ({ $count })
sftp-transfer-panel-close-tooltip = Close transfer panel
sftp-drop-to-upload = Drop files to upload
sftp-error-channel-failed = Failed to open SFTP channel: { $error }
sftp-error-connection-cancelled = Connection cancelled
sftp-error-server-config-missing = Server configuration not found
sftp-error-server-config-read = Failed to read server configuration: { $error }
sftp-error-not-connected = Not connected to the server
sftp-error-list-dir = Failed to list directory: { $error }
sftp-error-delete = Delete failed: { $error }
sftp-error-illegal-filename = The file name contains invalid characters
sftp-error-name-empty = The name cannot be empty
sftp-error-name-separator = Invalid name: it cannot contain a path separator
sftp-error-rename = Rename failed: { $error }
sftp-error-folder-name-empty = The folder name cannot be empty
sftp-error-create-folder = Failed to create folder: { $error }
sftp-error-invalid-target-path = The target path is not valid
...
byop-placeholder-no-provider = No custom provider configured — add one in Settings → AI
byop-placeholder-not-configured = Not configured
workflow-autofill-requires-byop = Autofill requires a BYOP model. Configure a provider and model in Settings → AI.
ai-plan-saved-locally-tooltip = The plan has been saved locally automatically.
workspace-log-export-header = Zap log export
workspace-log-export-generated-at = Generated at: { $time }
workspace-log-export-version = Version: { $version }
workspace-log-export-channel = channel: { $channel }
workspace-log-export-execution-mode = Execution mode: { $mode }
workspace-log-export-log-dir = Log directory: { $dir }
sftp-error-upload = Upload failed: { $error }
sftp-error-upload-not-connected = Upload failed: not connected to the server
sftp-error-download = Download failed: { $error }
sftp-error-download-not-connected = Download failed: not connected to the server
sftp-ops-error-connection = Connection error: { $detail }
sftp-ops-error-operation = Operation error: { $detail }
sftp-ops-error-local-io = Local IO error: { $detail }
sftp-ops-error-no-credentials = Credentials not found: { $detail }
sftp-ops-error-cancelled = Transfer cancelled
cloud-sync-error-crypto = Encryption error: { $detail }
cloud-sync-error-gist = Gist error: { $detail }
cloud-sync-error-provider = Data provider error: { $detail }
cloud-sync-error-serialization = Serialization error: { $detail }
cloud-sync-error-version-store = Version store error: { $detail }
cloud-sync-gist-error-request = Network request failed: { $detail }
...
settings-about-cef-title = CEF kernel
settings-about-cef-description = Checks the upstream CEF build index. Upgrading is a manual rebuild (bump the cef crate, re-embed the framework, re-sign); see CEF-UPGRADE.md.
settings-about-cef-installed = Kernel { $version }
settings-about-cef-up-to-date = Up to date ({ $version })
settings-about-cef-available = { $latest } available (current { $installed })
settings-about-cef-failed = Check failed: { $error }
settings-about-cef-unknown = Unable to compare versions
