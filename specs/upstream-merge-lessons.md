...
> **✅ 恢复流程修复（2026-09-12）**：移植 `a2f586584`(#10423)——修「历史会话里点命令行不展开命令内容块」。根因是恢复期命令块与 AI 块**不相邻**（旧 `create_restored_command_block` push 到末尾，而 AI 块插在活动块之前）；上游改为「先按序插入持久化命令块（复用活动块）→ 按时间戳算 command_block_index → AI 块插到其前」并删除旧函数。本地适配：本仓 proto 无 `start_ts/finish_ts` 故「命令运行时长」未移、保留本地 CLI subagent 机制与插入去重。详见 history §43
