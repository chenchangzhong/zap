...
> **⚠️ 实测否决（2026-09-12）**：`4b894db80`（编译期 serde 优化 #15455）阶段 1 已落地（提交 `e11deb51e`）但**实测净收益为负**——`warp` crate 编译 98.6s → 100.7s（+2.1%，同态抖动仅 ±0.2s），机制层确实减少 62% `ContentDeserializer`/41% `__DeserializeWith` 实例化但 rlib 仅小 0.2%。终局：整项不做、阶段 2 不继续，提交可 `git revert` 回滚。**测量必须 `RUSTC_WRAPPER=` 绕过 sccache**（本仓强制 sccache，`touch` 只改 mtime 会命中缓存导致 wall-time 失真：同一命令实测 23s/117s/139s）。详见 history.md §38
