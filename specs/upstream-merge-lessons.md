...
> **✅ 收尾三项（2026-09-12）**：① 删除死代码 `AIBlock::output_model_display_name`；② 合并 `5e7030db7`（warping 行显示模型名 #15323，**门控改为本地运行时 flag 且 zap_oss 默认开启**——上游是 Cargo feature + DOGFOOD 双层门控）；③ 合并 `142b87102`（Attach file 成为**默认不绑键**的命令面板动作 #15762）。详见 history.md §40
