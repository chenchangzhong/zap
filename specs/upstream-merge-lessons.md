...
> **✅ 追加移植（2026-09-12 三波）**：右键行为设置 `c25ac4070`（#15365）+ Shift+右键提示 `18179177a`（#15392）已落地——设置项 `terminal.input.right_click_behavior`（菜单默认 / 直接粘贴），块列表、alt screen、输入行等 6 处右键面接入，`warpui_core::EventHandler::on_right_mouse_down` 增加 `&ModifiersState`（详见 history.md §37）
> **⚠️ 实测否决（2026-09-12）**：`4b894db80`（编译期 serde 优化 #15455）阶段 1 已落地（提交 `e11deb51e`）但**实测净收益为负**——`warp` crate 编译 98.6s → 100.7s（+2.1%，同态抖动仅 ±0.2s），机制层确实减少 62% `ContentDeserializer`/41% `__DeserializeWith` 实例化但 rlib 仅小 0.2%。终局：整项不做、阶段 2 不继续，提交可 `git revert` 回滚。**终局：阶段 1 保留（用户决定，提交 `e11deb51e` 留在 main）、阶段 2（`Artifact` + `AIAgentContext`/`AIAgentAttachment`）不做**。**测量必须 `RUSTC_WRAPPER=` 绕过 sccache**（本仓强制 sccache，`touch` 只改 mtime 会命中缓存导致 wall-time 失真：同一命令实测 23s/117s/139s）。详见 history.md §38
> **✅ i18n 补齐（2026-09-12）**：本轮移植的 4 处硬编码英文文案已全部中文化（右键行为下拉、提问时间戳 tooltip 与格式、Ctrl+Tab 行为/全局热键下拉）——**移植上游 UI 功能务必顺带检查文案是否走 i18n**（范式：`-> String` + `crate::t!`；时间/占位格式按 `crate::i18n::current_languages()` 分支）。详见 history.md §39
> **✅ 收尾三项（2026-09-12）**：① 删除死代码 `AIBlock::output_model_display_name`；② `5e7030db7`（warping 行显示模型名 #15323）**已试并回滚**——本仓从不构造 `Message::ModelUsed`（BYOP 忽略该消息），`output.model_info` 恒为 None，特性不可能生效（见 history.md §40.7）；③ 合并 `142b87102`（Attach file 成为**默认不绑键**的命令面板动作 #15762）。详见 history.md §40
> **状态入口**：`specs/upstream-merge-status.md`——已合并 31 / 未合并 5 / 本地补齐 4 / 淘汰 53 的唯一总表（本文件只放教训）
> **✅ 恢复流程修复（2026-09-12）**：移植 `a2f586584`(#10423)——修「历史会话里点命令行不展开命令内容块」。根因是恢复期命令块与 AI 块**不相邻**（旧 `create_restored_command_block` push 到末尾，而 AI 块插在活动块之前）；上游改为「先按序插入持久化命令块（复用活动块）→ 按时间戳算 command_block_index → AI 块插到其前」并删除旧函数。本地适配：本仓 proto 无 `start_ts/finish_ts` 故「命令运行时长」未移、保留本地 CLI subagent 机制与插入去重。详见 history §43。**后续 `173ea34e4`**：移植后仍错位，逐层对比后发现差异在**数据**——本地 BYOP 消息全写 `timestamp: None`（上游来自服务端自带时间戳）⇒ 恢复时 exchange.start_time/命令块 start_ts 退化成同一值，上游时间戳配对失效；已补消息时间戳（新会话）＋旧会话兼容回退（本地 shim，判据=epoch 或不同时间值×2≤交换数）。教训：**对比参考实现要连它的数据前提一起比**；**写判据前先量数据形态**（§43.7）
