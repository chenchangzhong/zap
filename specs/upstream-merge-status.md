...
| **恢复流程：#10423** | `1d5996d5a` | `a2f586584`（修复「恢复历史会话时命令块与 AI 块错位 → 点命令行展开无效」）。新增 `load_ai_conversation_tests.rs`（14 用例全绿）；本地适配：剥离 proto 时间戳（本仓 fork 无 `start_ts/finish_ts`，「命令运行时长」未移植）、保留本地 CLI subagent 快照机制与去重、cloud mode 插桩取本地侧。详见 history §43 |
...
4. **编译期栈的其余部分是否补做**（见 §八）：栈外的 `1e4b86a81` (#15517) **已合并**（`3dd415c92`）；
   栈内 #15453 / #15454 未做，`4b894db80` 自身还剩「阶段 2」（2 文件 / 4 处冲突）。建议：其余都不做，
...
**栈外同期的编译期提交**：`1e4b86a81` (#15517 release-cli codegen-units 1→4，**已合并** `3dd415c92`)、
`21f413b79` (#14875 终端 grid 大搬运，已在淘汰 53)、`2a183d552` (#15462 抽 `secret_redaction` crate，已在淘汰 53)。

**结论**：栈内收益基本测不出（stage 1 反向），栈外唯一能测出收益的 `1e4b86a81` 已合并；
