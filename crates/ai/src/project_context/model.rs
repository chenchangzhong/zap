    ...
    fn register_watcher_for_path(&self, path: &Path, ctx: &mut ModelContext<Self>) {
        ...
        ctx.spawn_stream_local(
            ...
            move |me, update, ctx| {
                ...
                if let Some(queued) = me.pending_updates.get_mut(&repo_path) {
                    queued.push(update);
                    return;
                }

                // Read the current rules without removing them from the map so a
                // concurrent task can't drop updates that arrive while we're in flight.
                let Some(rules) = me.path_to_rules.get(&repo_path).cloned() else {
                    return;
                };

                // Mark this path as having an in-flight task (empty queue).
                me.pending_updates.insert(repo_path.clone(), Vec::new());
