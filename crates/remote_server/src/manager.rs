...
#[cfg(not(target_family = "wasm"))]
use crate::transport::{Connection, ControlPath, InstallSource, RemoteTransport};
use crate::HostId;
...
impl RemoteServerManager {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            ctx.background_executor()
                .spawn(async move {
                    ...
                    // check sequentially so that each step reuses the same
                    // SSH ControlMaster connection instead of opening
                    // parallel channels. The old-binary check lets the
    ...
    /// Emits `BinaryInstallComplete { result, install_source }`.
    ///
