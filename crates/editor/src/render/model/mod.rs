    ...
    fn handle_layout_action(&mut self, action: LayoutAction, ctx: &mut ModelContext<Self>) {
        match action {
            ...
            LayoutAction::ScrollToFraction {
                ...
                // 延迟到 `apply_element_update` 应用：此处读取内容高度会拿到未渲染编辑器的
                // 陈旧值（新 pane 尚无 viewport 尺寸）而被夹到顶部；`minimum_version` 由提交方
                // 给出，因为产生内容的编辑可能在本次 action 之后才到达布局通道。
                self.pending_scroll_fraction = Some(PendingScrollFraction {
                    fraction,
                    minimum_version,
                });
                // 触发一次重渲染，使内容已静止时后续的 element layout 仍能应用该 fraction。
                ctx.notify();
            }
