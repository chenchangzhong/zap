...
任何"这是 bug / 这里有风险 / 这需要修"的结论,必须附带下面两项:

1. **触发条件** —— 能让它发生的具体输入或调用路径。
2. **验证证据** —— 实际跑过的测试、命令及其输出。

两项缺一,结论必须显式标注为 **「未验证推测」**,不得进入待修清单、
不得据此排优先级。
...
| 模式 | 例子 | 正确做法 |
|------|------|----------|
| 用错命令得出错数字 | `git rev-list HEAD..upstream/master` 在无 merge-base 的 fork 上返回 1795,真实待评估只有 4 | 用 `specs/upstream-merge-lessons.md` 记录的同步边界 hash |
| 搜错符号名当成"不存在" | 搜 `unregister_session` 返回空即断言无生命周期钩子,真实名字是 `remove_session` | 先确认符号存在,再查调用方;查不到先怀疑名字 |
| 只读局部就断言崩溃 | 断言 `params[1]` 越界 panic,实际函数入口已有 `params.is_empty()` 守卫 | 读完整函数,包括入口守卫与早返回 |
| 忽略依赖层已有的约束 | 断言 OSC payload 无上限,实际 vte 有 `MAX_OSC_PARAMS = 16` 硬上限 | 追到依赖实现,别停在本仓代码 |

#### 检查清单

- 说"缺少某个调用/兜底"前:符号名对吗?换个名字再搜一次了吗?
- 说"会 panic / 越界"前:入口守卫读了吗?能写出触发输入吗?
- 说"无上限 / 无校验"前:依赖库那一层查了吗?
- 排优先级前:每一条都验证过吗?没验证的先标注,别混进清单。
