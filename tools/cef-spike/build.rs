// 编译阶段 1 探针的 ObjC 宿主视图(probes/hole_probe.m 的洞拓扑 +
// probes/osr_host.m 的 OSR 宿主视图 —— 后者把 CEF 的 IOSurface 贴进 CALayer,供 T1 透明实验)。
...
fn main() {
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
        ...
        println!("cargo:rerun-if-changed=probes/osr_host.m");
    }
}
